...
import { submitDonation } from '@/app/(frontend)/actions'
...
export function DonationSection({ content }: DonationSectionProps) {
  ...
  const title = content?.title || 'Make a Donation'
  const subtitle =
    content?.subtitle || 'Your support helps families in Lee County.'
  const successTitle = content?.successTitle || 'Thank You!'
  const successMessage =
    content?.successMessage ||
    'Your donation has been recorded. We truly appreciate your generosity.'

  const donationAmounts = content?.presetAmounts
    ? content.presetAmounts
        .split(',')
      ...
      <div className="container-custom">
        <div className="max-w-xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="card">
            ...
            <button
              type="submit"
              disabled={isPending || getAmount() <= 0}
